...
class TestRoutes(TestCase):
    ...
    def test_csv_exists(self, CSV):
        ...
        CSV.assert_called_once_with(
            client=ctx.client, bucket="TEST_BUCKET", key="TEST_KEY.csv"
        )
    ...
    def test_json_exists(self, JSON):
        ...
        JSON.assert_called_once_with(
            client=ctx.client, bucket="TEST_BUCKET", key="TEST_KEY.json"
        )
    ...
    def test_parquet_exists(self, Parquet):
        ...
        Parquet.assert_called_once_with(
            client=ctx.client, bucket="TEST_BUCKET", key="TEST_KEY.parquet"
        )
    ...
    def test_sqlite(self, Sqlite):
        ...
        Sqlite.assert_called_once_with(
            client=ctx.client, bucket="TEST_BUCKET", key="TEST_KEY.sqlite"
        )
